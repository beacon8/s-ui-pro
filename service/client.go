...
func (s *ClientService) GetAll() (*[]model.Client, error) {
	...
	err := db.Model(model.Client{}).
		Select("`id`, `enable`, `name`, `desc`, `group`, `remark`, `inbounds`, `up`, `down`, `volume`, `expiry`, `created_at`, `online_at`, `up_limit`, `down_limit`, `limit_unit`").
		Scan(&clients).Error
	if err != nil {
		return nil, err
	}
...
func (s *ClientService) findInboundsChanges(tx *gorm.DB, client *model.Client, fillOmitted bool) ([]uint, error) {
	...
	if fillOmitted {
		...
		client.OnlineAt = oldClient.OnlineAt
		...
		client.TotalUp = oldClient.TotalUp
		client.TotalDown = oldClient.TotalDown
	}
